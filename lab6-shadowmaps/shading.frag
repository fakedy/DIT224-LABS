...
vec3 calculateDirectIllumiunation(vec3 wo, vec3 n, vec3 base_color)
{
	...
	vec3 lightDistanceVector = viewSpaceLightPosition - viewSpacePosition;
	float lightDistance = length(lightDistanceVector);

	float DSquared = lightDistance*lightDistance;

	vec3 Li = point_light_intensity_multiplier * point_light_color / DSquared;

	vec3 wi = normalize(lightDistanceVector);
	float lightFactor = dot(n, wi);
	...
	///////////////////////////////////////////////////////////////////////////
	vec3 diffuse_term = (base_color/PI) * lightFactor * Li;
	...
	float woDotwh = max(dot(wo,wh), 0.001); // avoid negative denominator
	...
	// G
	float G = min(1, min( 2*(dot(n, wh)*dot(n,wo) / woDotwh), 2*(dot(n, wh)*dot(n,wi) / woDotwh)));

	float denominator = 4 * max(dot(n,wo)*dot(n,wi), 0.001);
	...
	vec3 dielectric_term = BRDF * lightFactor * Li + (1.0 - F) * diffuse_term;
	vec3 metal_term = BRDF * base_color * lightFactor * Li;
...
vec3 calculateIndirectIllumination(vec3 wo, vec3 n, vec3 base_color)
{
	...
	lookup = vec2(phi / (2.0 * PI), 1.0 - theta / PI);
	...
	float F = material_fresnel + (1.0 - material_fresnel) * pow((1 - woDotwh), 5);
